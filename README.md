...
| `EMU2_KBHIT_CALLS` | Number of `kbhit()` polling calls allowed in time limit for tight-loop detection. If more than `EMU2_KBHIT_CALLS` execute in less than `EMU2_KBHIT_TIME` microseconds, the CPU sleeps for 10ms.<br><br>Default is `1000`. Set to `0` to *completely disable* the keyboard throttling (otherwise idle CP/M and DOS programs may consume 100% host CPU, but *may* be more responsive). |
