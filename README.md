...
| `EMU2_CPM_APPEND` | Sets a list of CP/M‑86 drive letters (e.g. `D:;E:`) to search for *data* files on open for CP/M‑86 programs.  Works very much like the DOS `EMU2_APPEND` option, but for CP/M‑86 programs. This maps one or more drives to appear as part of the current drive for programs that search for and open *data* files. |
